...
export default function VideoCall({ open, handleClose }) {
  const [muteAudio, setMuteAudio] = useState(false)
  const [muteVideo, setMuteVideo] = useState(false)

  const handleToggleAudioCall = () => {
      ...
      <div className='w-full max-w-142.5 rounded-lg bg-white dark:bg-boxdark md:py-8 px-8 py-12'>
        <div className='flex flex-col space-y-6'>
          ...
          <div className='grid grid-cols-2 gap-4 h-50 mb-4'>
            {/* User 1 */}
            <div className='relative h-full w-full bg-gray rounded-md dark:bg-boxdark-2 flex items-center justify-center'>
              <div className='space-y-2'>
                <img src={User01} alt='' className='h-20 w-20 rounded-full object-center object-cover' />
                <div className='font-medium text-sm text-center'>Tao</div>
              </div>
            ...
            <div className='relative h-full w-full bg-gray dark:bg-boxdark-2 rounded-md flex items-center justify-center'>
              <div className='space-y-2'>
                ...
                <div className='font-medium text-sm text-center'>Mày</div>
              </div>
          ...
          <div className='flex flex-row items-center justify-center space-x-4'>
            {/* Microphone Button */}
            <button onClick={handleToggleAudioCall} className='p-3 rounded-md bg-gray dark:bg-boxdark text-black dark:text-white hover:bg-opacity-80 flex items-center justify-center'>
            ...
            {/* Video Camera button */}
            <button onClick={handleToggleVideoCall} className='p-3 rounded-md bg-gray dark:bg-boxdark text-black dark:text-white hover:bg-opacity-80 flex items-center justify-center'>
